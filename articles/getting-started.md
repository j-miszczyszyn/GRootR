...
Convert root node points into line segments and compute all geometric
metrics:
...
This creates an `sf` object where each row is a line segment between two
...
spatially matches segments within `max_dist` meters of a tree point.
[`propagate_tree_name()`](https://j-miszczyszyn.github.io/GRootR/reference/propagate_tree_name.md)
then ensures ALL segments of the same `ROOT_ID` inherit the tree
identity, even if only one segment was close enough to match.
...
Build polygons representing the horizontal spread of each tree’s root
system:
