...
GRootR expects a CSV file exported from GPR processing software (e.g., REFLEXW, GPR-SLICE, or similar). Each row represents a single node (point) along a detected root, with the following columns:
...
Note that `ROOT_ID` is only specified in the first row of each root — subsequent rows are empty (NA). GRootR fills these automatically.
...
GRootR does not reproject data — it uses whatever CRS your coordinates are in. You specify the EPSG code when building segments:
...
**Important:** Use a *projected* CRS (meters), not geographic (degrees). Distances and areas are computed in CRS units.
...
`split_survey_column()` splits it by `_` and extracts one part as the plot ID:
...
If you don't have a Survey column or don't need it, pass `survey_col = NULL` to `prepare_root_data()`.
...
For joining roots with trees, you need a spatial file (shapefile, GeoPackage, etc.) with tree stem positions as **points**:
...
The tree data must have an **ID column** (any name) that uniquely identifies each tree. CRS is automatically matched during the join.
